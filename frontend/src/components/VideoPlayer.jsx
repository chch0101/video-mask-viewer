...
const VideoPlayer = forwardRef(function VideoPlayer({
  ...
  }, [viewingMosaic, maskSettings.visible, maskSettings.opacity, maskSettings.blendMode, videoUrls.mosaic, selectedMaskSource])
  ...
  useEffect(() => {
    ...
    const handleMaskReady = () => {
      ...
      // 비디오 변경 시 브라우저가 playbackRate를 1로 리셋하므로 복원
      if (maskVideo && playbackRateRef.current !== 1) {
    ...
    const handleSourceData = () => {
      ...
      // 비디오 src 변경 시 브라우저가 playbackRate를 1로 리셋하므로 복원
      if (sourceVideo && playbackRateRef.current !== 1) {
        sourceVideo.playbackRate = playbackRateRef.current
      }
      updateDebugInfo()
      requestAnimationFrame(() => drawFrameRef.current?.())
    }
    ...
    if (onSeekToFrame) {
      ...
      return (
        ...
        {viewingMosaic && currentVideo && (
          <video
            ref={mosaicVideoRef}
            src={videoUrls.mosaic || (selectedMaskSource
        ...
        {sourceConverting && !videoPreparing && !viewingMosaic && (
          <div className="source-converting">
