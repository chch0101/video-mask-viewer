...
def get_s3_cache():
    ...
    with S3_CACHE_LOCK:
        ...
        try:
            ...
            prefix_mosaics = f"{S3_PREFIX}/mosaic/"
            source_counts = {}
            ...
            for page in paginator.paginate(Bucket=S3_BUCKET, Prefix=prefix_mosaics):
                for obj in page.get('Contents', []):
                    key = obj['Key']
                    s3_files_set.add(key)
                    if key.endswith('.mp4'):
                        # 패턴: video/mosaic/source_name/filename.mp4
                        parts = key.replace(prefix_mosaics, '').split('/')
                        if len(parts) == 2:
                            source_name = parts[0]
                            source_counts[source_name] = source_counts.get(source_name, 0) + 1
        ...
        if USE_S3 and s3_files_set:
            for s3_source in s3_cache.get('sources', []):
                ...
                if source_name not in available:
                    ...
                    s3_mosaic_key = f"{S3_PREFIX}/mosaic/{source_name}/{filename}"
                    if s3_key in s3_files_set or s3_mosaic_key in s3_files_set:
                        available.append(source_name)
...
def serve_masks_video(source, filename):
    ...
    if not os.path.exists(original_path):
        ...
        if USE_S3:
            ...
            s3_mosaic_key = f"{S3_PREFIX}/mosaic/{source}/{filename}"
            cache = get_s3_cache()
            s3_exists = ('_s3_files_set' in cache and s3_key in cache['_s3_files_set']) or s3_file_exists(s3_key)
            if s3_exists:
                url = get_s3_presigned_url(s3_key)
            ...
            s3_mosaic_exists = ('_s3_files_set' in cache and s3_mosaic_key in cache['_s3_files_set']) or s3_file_exists(s3_mosaic_key)
            if s3_mosaic_exists:
                url = get_s3_presigned_url(s3_mosaic_key)
                if url:
                    print(f"[S3 Redirect] mosaic/{source}/{filename} (served as mask)")
                    return redirect(url)
        return jsonify({'error': 'File not found'}), 404
...
def get_video_urls(name):
    ...
    if USE_S3 and s3_client:
        ...
        if mask_source:
            ...
            mask_mosaic_key = f"{S3_PREFIX}/mosaic/{mask_source}/{name}.mp4"
            mask_local = os.path.join(VIDEO_DIR, 'masks', mask_source, f'{name}.mp4')
        else:
            mask_key = f"{S3_PREFIX}/mask/{name}.mp4"
            mask_mosaic_key = f"{S3_PREFIX}/mosaic/{name}.mp4"
        ...
        if not os.path.exists(mask_local):
            ...
            if s3_exists:
                ...
                if url:
                    urls['mask'] = url
            elif mask_mosaic_key:
                s3_mosaic_exists = ('_s3_files_set' in cache and mask_mosaic_key in cache['_s3_files_set']) or s3_file_exists(mask_mosaic_key)
                if s3_mosaic_exists:
                    url = get_s3_presigned_url(mask_mosaic_key)
                    if url:
                        urls['mask'] = url

        # 모자이크 모드일 경우 직접 S3 URL을 제공하기 위한 추가 로직
        if mask_mosaic_key:
            s3_mosaic_exists = ('_s3_files_set' in cache and mask_mosaic_key in cache['_s3_files_set']) or s3_file_exists(mask_mosaic_key)
            if s3_mosaic_exists:
                url = get_s3_presigned_url(mask_mosaic_key)
